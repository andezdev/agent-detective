...
description: Monorepo setup with pnpm and Turbo — build, test, lint, and run locally.
sidebar:
  order: 1
---
...
**Agents and quick verify:** [Agent harness](./agent-harness.mdx) · **Do / don't / pitfalls:** [Agent golden rules](./agent-golden-rules.md) · **Root index:** [AGENTS.md](../../AGENTS.md)
...
<Tabs syncKey="run-mode">
  <TabItem label="Development">
    ...
    pnpm run dev
    ```
    Uses `tsx` watch — auto-restarts on changes.
  </TabItem>
  <TabItem label="Production">
    ```bash
    pnpm start
    ```
    Requires `pnpm run build:app` first.
  </TabItem>
</Tabs>
...
- **Dependency upgrades:** Bump shared tools and runtime libraries in the **`catalog:`** section of [`pnpm-workspace.yaml`](../../pnpm-workspace.yaml), then `pnpm install`, so every package that references `"catalog:"` for that name updates together. Dependencies that are not in the catalog (for example Astro or Tailwind under [`apps/`](../../apps/), or `pino` under [`packages/observability`](../../packages/observability)) are upgraded in that package's own `package.json`. After any bump, run `pnpm run build`, `pnpm test`, and `pnpm run lint`. Use `pnpm outdated -r` to see newer versions across the workspace.
...
- **`pnpm run build:app`** runs **tsup** on the root entrypoint and writes **`dist/index.js`** for `pnpm start` (run **`pnpm run build`** first so workspace packages are built).
...
- **npm publish** is automated by **release-please** on merge of the Release PR (see [publishing.md](../plugins/publishing.md)); do not publish manually from a dirty tree unless debugging.
...
| Release PR merge | **release-please** tags and publishes `agent-detective` + `@agent-detective/*` (see [publishing.md](../plugins/publishing.md)) |
...
- src/ Main application (TypeScript)
  ...
  - agents/ AI agent integrations
    ...
    - cursor.ts
    - utils.ts
  - server.ts Fastify server (+ Core API endpoints)
  - index.ts Bootstrap
- packages/ Workspace packages
  - types/ @agent-detective/types
  - sdk/ @agent-detective/sdk
...
:::tip[Mock mode]
When `mockMode: true`, the adapter logs `[MOCK] Added comment...` instead of calling Jira.
:::

When `mockMode: false`, set Jira credentials via config or `JIRA_*` env (see [configuration.md](../config/configuration.md)).

**Smoke tests:** [Agent harness — plugin / integration smoke](./agent-harness.mdx) (`pnpm run jira:webhook-smoke`, `pnpm run ci:webhook-smoke`). **npm operators:** **`agent-detective smoke`** — [get started](../operator/get-started.md).
...
**Plugin fails to load** — Check the console output; plugins log warnings instead of crashing.

**Repository not found** — Verify `config/default.json` has correct repo paths and the agent has read permissions.

**Agent not responding** — Check the agent is installed and on PATH. Try running standalone: `echo "hello" | opencode exec "say hi"`
:::
...
```typescript title="Plugin imports"
